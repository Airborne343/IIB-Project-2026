...
def coupled_oscillatory_model(t, state):
    ...
    #eta_ddot equation
    eta_ddot = - ((np.abs(omega_0) ** 2) * eta) - ((-(rho * (c_gas**2) * psi_mag * trachea_area)/(psi_norm_squared)) * u_dot) + (2 * nu * eta_dot) - (kappa * (eta ** 2) * eta_dot)
    
    #u_ddot equation
    u_ddot = (-beta * (float(U(t)) + u) * u_dot - (abs(float(dU(t))) * beta + ((c_gas**2 * area[33])/(V_h(t)*l_m[33]))) * u - gamma * eta_dot)
